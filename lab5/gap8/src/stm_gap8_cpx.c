...
static CPXPacket_t packet_control;
void commande_task(void *parameters)
{
  while (1)
  {
...
void start_example(void)
{
  ...
  xTask = xTaskCreate(commande_task, "control_task", configMINIMAL_STACK_SIZE * 2,
                      NULL, tskIDLE_PRIORITY + 1, NULL);
  if (xTask != pdPASS)
  {
    cpxPrintToConsole(LOG_TO_CRTP, "control task did not start !\n");
    pmsis_exit(-1);
  }
